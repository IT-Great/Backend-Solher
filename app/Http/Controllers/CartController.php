...
//         // 👇 [PERBAIKAN] Tidak butuh LockForUpdate di keranjang. Cukup validasi stok normal. 👇
...
class CartController extends Controller
{
    private function calculateCartTotals($cartItems, $currency = 'IDR')
    {
        ...
        // 👇 [PERBAIKAN] Tarik Data Kurs dari Cache 👇
        $rates = Cache::get('exchange_rates', []);
        $exchangeRate = ($currency !== 'IDR') ? ($rates[$currency] ?? 1) : 1;
        ...
        foreach ($groupedItems as $groupKey => $group) {
            if ($groupKey === 'NO_PROMO') {
                foreach ($group['items'] as $item) {
                    $totalPrice += ($item->quantity * $this->resolveProductPrice($item->product, $currency, $now, $exchangeRate));
                }
                continue;
            }
            ...
            foreach ($items as $item) {
                $normalTotalGroup += ($item->quantity * $this->resolveProductPrice($item->product, $currency, $now, $exchangeRate));
            }

            if ($type === 'bundle') {
                // 👇 [PERBAIKAN] Fallback yang aman dengan konversi kurs 👇
                $fallbackPriceIdr = $conf['price']['IDR'] ?? 0;
                $bundlePrice = $conf['price'][$currency] ?? ($fallbackPriceIdr * $exchangeRate);
                ...
                $sortedItems = $items->sortByDesc(function ($item) use ($currency, $now, $exchangeRate) {
                    return $this->resolveProductPrice($item->product, $currency, $now, $exchangeRate);
                });

                $assignedRemainder = 0;
                foreach ($sortedItems as $item) {
                    $normalPrice = $this->resolveProductPrice($item->product, $currency, $now, $exchangeRate);
                    if ($assignedRemainder < $remainderQty) {
                        $take = min($item->quantity, $remainderQty - $assignedRemainder);
                        $groupPromoPrice += ($take * $normalPrice);
                        $assignedRemainder += $take;
                ...
                // 👇 [PERBAIKAN] Konversi Syarat Minimum & Maksimum ke Mata Uang Target 👇
                $minPurchaseBase = $conf['min_purchase'] ?? 0;
                $maxDiscountBase = $conf['max_discount'] ?? 0;

                $minPurchase = $minPurchaseBase * $exchangeRate;
                $maxDiscount = $maxDiscountBase * $exchangeRate;
        ...
        return [
            // Gunakan pembulatan 2 desimal jika bukan IDR untuk presisi
            'total_price' => $currency === 'IDR' ? round($totalPrice) : round($totalPrice, 2),
            'total_discount' => $currency === 'IDR' ? round($totalDiscount) : round($totalDiscount, 2),
        ];
    }

    private function resolveProductPrice($product, $currency, $now, $exchangeRate = 1)
    {
        $prices = is_string($product->prices) ? json_decode($product->prices, true) : ($product->prices ?? []);
        $discountPrices = is_string($product->discount_prices) ? json_decode($product->discount_prices, true) : ($product->discount_prices ?? []);

        // 👇 [PERBAIKAN] Jika Harga Mata Uang Target kosong, konversi dari Harga Dasar IDR 👇
        $basePrice = $prices[$currency] ?? ($product->price * $exchangeRate);
        $discountPrice = $discountPrices[$currency] ?? (!empty($product->discount_price) ? $product->discount_price * $exchangeRate : null);
    ...
    public function store(Request $request)
    {
        ...
        // Simpan database raw/gross amount dalam IDR
        $price = $this->resolveProductPrice($product, 'IDR', now(), 1);
    ...
    public function update(Request $request, $id)
    {
        ...
        // Simpan database raw/gross amount dalam IDR
        $price = $this->resolveProductPrice($cart->product, 'IDR', now(), 1);
