...
{
    // =========================================================================
    // HELPER: Mengambil ID Admin dari Cache (Menghilangkan N+1 / Kueri Berulang)
    // =========================================================================
    private function getCachedAdminIds()
    {
        return Cache::remember('admin_superadmin_ids', 86400, function () {
            return User::whereIn('usertype', ['admin', 'superadmin'])->pluck('id')->toArray();
        });
    }
    ...
    public function getAdmins()
    {
        ...
        if ($mainAdmin) {
            ...
            $adminIds = $this->getCachedAdminIds();
    ...
    public function getMessages($userId)
    {
        ...
        // 👇 PERBAIKAN FATAL: Membatasi pesan max 100 untuk mencegah Memory Leak (OOM) 👇
        $query = Message::with('sender');

        if ($isCustomer) {
            $query->where(function ($q) use ($myId, $adminIds) {
                $q->where('sender_id', $myId)->whereIn('receiver_id', $adminIds);
            })->orWhere(function ($q) use ($myId, $adminIds) {
                $q->whereIn('sender_id', $adminIds)->where('receiver_id', $myId);
            });
        } else {
            $query->where(function ($q) use ($userId, $adminIds) {
                $q->whereIn('sender_id', $adminIds)->where('receiver_id', $userId);
            })->orWhere(function ($q) use ($userId, $adminIds) {
                $q->where('sender_id', $userId)->whereIn('receiver_id', $adminIds);
            });
        ...
        // Urutkan dari terbaru, ambil 100, lalu balikkan lagi urutannya agar chat rapi dari atas ke bawah
        $messages = $query->latest()->limit(100)->get()->reverse()->values();

        return response()->json($messages);
    }

    // 3. Menyimpan Pesan, Logika AI Handoff, & Smart Email Notification
    public function sendMessage(Request $request, FileUploadService $fileUpload)
    {
        $request->validate([
            'receiver_id' => 'required|exists:users,id',
        ...
        if ($request->hasFile('attachment')) {
            ...
            // 👇 PERBAIKAN INFRASTRUKTUR: Gunakan S3/FileUploadService agar mendukung arsitektur multi-server 👇
            try {
                $attachmentPath = $fileUpload->uploadToS3($file, 'chat_attachments');
            } catch (\Exception $e) {
                Log::error('Upload chat attachment gagal: ' . $e->getMessage());
    ...
    public function typing(Request $request)
    {
        // 👇 PERBAIKAN: Pastikan input divalidasi agar tidak error 500 👇
        $request->validate([
            'receiver_id' => 'required|exists:users,id'
        ]);
