...
//             if ($type === 'bundle') {
//                 $bundlePrice = $conf['price'][$currency] ?? ($conf['price']['IDR'] ?? 0);
//                 $bundleQty = max(1, $group['bundle_qty']); // Cegah division by zero
//                 $totalQty = $items->sum('quantity');

//                 // [ANTI-BUG] Jika harga bundle 0 karena salah ketik di Admin, fallback ke harga normal!
...
class CalculateCartTotalsAction
{
    public function execute(User $lockedUser, $cartItems, Request $request, PromoMerdekaService $promoService): array
    {
        ...
        // 👇 [PERBAIKAN BARU] KALKULASI DISKON TIER PRIVILEGE (ÉLAN 5% / HÉRITAGE 10%) 👇
        // Diskon Tier dikenakan sebelum potongan Voucher dan Poin
        ...
        // 👇 [PERBAIKAN FATAL] KALKULASI POIN LOYALITAS 👇
        // Hitung estimasi poin: Rp 1.000 = 1 Poin (dibagi 1.000, bukan 100.000)
        $earnedPoints = floor($totalAfterPromo / 1000);
        $pointsUsed = 0;

        if ($request->use_points > 0 && $lockedUser->is_membership) {
            // 👇 [PERBAIKAN] Batasi maksimal poin yang bisa dipakai = 5000 👇
