...
class DeductInventoryAction
{
    public function execute($transaction, $cartItems, array $finalItemPrices)
    {
        // 👇 PERBAIKAN: Urutkan berdasarkan product_id agar MySQL Lock konsisten dan mencegah Deadlock
        $sortedItems = collect($cartItems)->sortBy('product_id');
        ...
        foreach ($sortedItems as $item) {
            ...
            if ($remainingQuantityToDeduct > 0) {
                $activeBatches = ProductStock::where('product_id', $product->id)
                    ->where('quantity', '>', 0)
                    ->orderBy('created_at', 'asc')
                    ->lockForUpdate() // Aman karena parent Product sudah dilock
                    ->get();
