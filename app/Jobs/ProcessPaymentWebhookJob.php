...
class ProcessPaymentWebhookJob implements ShouldQueue
{
    ...
    protected function executePaymentSuccess($externalId, $status, $paymentMethod)
    {
        ...
        DB::transaction(function () use ($externalId, $status, $paymentMethod) {
            // 👇 PERBAIKAN 1: BACA INFO PAYMENT DULU TANPA LOCK (Mencegah Deadlock Menyilang) 👇
            $paymentInfo = Payment::where('external_id', $externalId)->first();
            if (!$paymentInfo) return;

            // LALU LOCK TRANSACTION LEBIH DULU SESUAI URUTAN HIRARKI (Transaction -> Payment)
            $transaction = Transaction::lockForUpdate()->find($paymentInfo->transaction_id);
            if (!$transaction) return;

            $payment = Payment::where('transaction_id', $transaction->id)->lockForUpdate()->first();

            if ($status === 'PAID') {
                // 👇 PERBAIKAN 2: CEGAH LATE PAYMENT RACE CONDITION 👇
                if ($transaction->status === 'cancelled') {
                    // Skenario: Customer bayar telat, tapi sistem terlanjur membatalkan order dan me-refund poin/stok.
                    // Jangan dikirim ke kurir! Ubah jadi butuh refund manual.
                    $payment->update(['status' => 'PAID']);
                    $transaction->update(['status' => 'refund_manual_required']);
                    Log::critical("LATE PAYMENT DETECTED: Order {$transaction->order_id} dibayar setelah batal. Butuh Refund Manual.");
                    return;
                }

                if ($payment->status === 'PAID' || in_array($transaction->status, ['processing', 'completed', 'refund_manual_required'])) return;
                ...
                if ($transaction->status !== 'cancelled' && $transaction->status !== 'refund_manual_required') {
                    $payment->update(['status' => $status]);
                    $transaction->update([
                        'status' => 'cancelled',
                        'shipping_status' => 'cancelled',
                    ...
                    // 👇 PERBAIKAN 3: SORTING ID PRODUK SEBELUM RESTORE STOK 👇
